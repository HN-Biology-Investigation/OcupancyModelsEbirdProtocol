...
Loads all required libraries for spatial data handling (sf, terra), data wrangling (tidyverse), and occupancy modeling (unmarked).
...
Imports eBird detection and sampling tables, the Honduras grid layer, and landscape metrics (pland). Merges landscape metrics with the grid.
...
Selects relevant eBird fields, standardizes OBSERVATION.COUNT, and merges detection and non-detection records. Removes duplicates by summarizing counts per unique sampling event.
...
Transforms the cleaned dataset into an sf object using latitude/longitude and previews detections on a map
...
# Load detections with grid IDs

Reads a preprocessed file where each eBird checklist has already been spatially assigned to a grid cell.
...
# Join eBird data with landscape metrics

Adds pland metrics to each checklist based on the grid cell.
...
# Basic GLM modeling (exploratory)

Fits simple Poisson models to test relationships between detections and land cover (pland_1) before building the hierarchical model.
...
Cleans effort and temporal variables, filters checklists by quality criteria (effort, protocol, observers), extracts coordinates and months, and selects the overwintering period.
Creates visit-level long and wide formats, builds observation covariates (time, date), and summarizes landscape metrics per grid.
...
Creates the detection history matrix (y), observation covariates (obsCovs), and site covariates (siteCovs).
Packages them into an unmarkedFrameOccu object for occupancy modeling.
...
# 10. Fit occupancy model

Fits a single-season occupancy model with detection submodel (Date + Time) and occupancy driven by landscape structure (pland_2).

```{r,  warning=FALSE, message=FALSE}
Modelo1 <- occu(~Date + Time ~pland_3, data = Setchr_umf)
summary(Modelo1)
...
# 11. Predict and map occupancy

Predicts occupancy probability for all grid cells in Honduras, merges predictions back to spatial data, and generates a map of occupancy probabilities.
Exports the map as a high-resolution PNG.
